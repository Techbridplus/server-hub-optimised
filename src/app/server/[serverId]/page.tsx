...
export default function ServerPage() {
  ...
  const handleJoinServer = async () => {
    try {
      ...
      // Update server data
      const serverData = await axios.get<ServerWithMembers>(
  ...
  useEffect(() => {
    const loadServerData = async () => {
      try {
        // Fetch server data
        const serverRes = await axios.get<ServerWithMembers>(
          `/api/servers/${serverId}`
        );
  ...
  try {
    // Add the missing query parameter
    const response = await axios.get(`/api/servers/${serverId}/events?type=all`);
    ...
    const upcoming = events.filter(
      (event: Event) => event.startDate && new Date(event.startDate) > now
    );
    const past = events.filter(
      (event: Event) => event.endDate && new Date(event.endDate) < now
    );
              ...
              <AlertDialog>
                ...
                <AlertDialogContent>
                  <AlertDialogHeader>
                    ...
                    <AlertDialogDescription>
                      Are you sure you want to leave {server.name}? You will
                      need to request access again if you want to rejoin.
                    </AlertDialogDescription>
                  </AlertDialogHeader>
                  <AlertDialogFooter>
          ...
          >
            ...
            {Object.values(upcomingEvents).length == 0 && (
              <div className="flex flex-col items-center justify-center rounded-lg border border-dashed p-8 text-center">
                ...
                {hasEditRights && <CreateEventDialog serverId={serverId} />}
              </div>
            )}
            ...
            <>
              ...
              >
                <div className="flex items-center justify-between">
                  ...
                  {hasEditRights && (
                    <CreateAnnouncementDialog serverId={serverId} />
                  )}
                </div>
        ...
        <div className="mt-8 space-y-12 md:hidden">
          <section id="events" className="space-y-8 scroll-mt-16">
            <div className="flex items-center justify-between">
              <h2 className="text-2xl font-bold">Upcoming Events</h2>
              {hasEditRights && (
                <CreateEventDialog serverId={serverId} buttonSize="sm" />
              )}
            </div>
